...
# Replace the mentionsList rendering logic to ALWAYS render if there are items, overriding searchState blocking
old_render_logic = """
        {/* MENTIONS LIST */}
...
new_render_logic = """
        {/* MENTIONS LIST */}
        <div className="space-y-4">
          {/* Always show skeleton if loading and list is empty */}
          {loading && !mentionsList.length ? (
            ...
            <div className="space-y-4">
              {/* IF LIST IS NOT EMPTY, ALWAYS RENDER IT. Add top loading indicators instead of blocking. */}
              {loading && (
              ...
              {searchState === 'AUTO_SCAN_COMPLETED' && scanJobStatus && (
                <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800/50 rounded-lg p-3 mb-4 flex items-center gap-3">
                  <Sparkles className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
                  <span className="text-sm text-emerald-800 dark:text-emerald-300 font-medium">
                    Quét xong: tìm thấy {scanJobStatus.summary?.serpapi_result_count || 0}, thêm mới {scanJobStatus.summary?.new_mentions_created || 0}, bỏ qua {scanJobStatus.summary?.duplicates_skipped || 0} trùng lặp.
                  </span>
                </div>
              )}
              {/* Bulk Action Bar goes below inside the mapping loop container */}
"""

# We also need to fix the case when mentionsList is empty, after loading.
new_empty_logic = """
          ) : searchState === 'AUTO_SCAN_NO_RESULTS' ? (
            <div className="py-20 flex flex-col items-center justify-center text-center bg-white dark:bg-[#050A15] rounded-xl shadow-sm border border-gray-200 dark:border-white/10">
              <div className="w-16 h-16 bg-gray-100 dark:bg-white/10 rounded-full flex items-center justify-center mb-4">
...
# We also need to fix the bottom part of the old_render_logic
# The original code has `<div className="space-y-4">` then `{['AUTO_SCAN_STARTING'...` which is now handled at the top
old_extra_scan_logic = """              {['AUTO_SCAN_STARTING', 'AUTO_SCAN_RUNNING'].includes(searchState) && (
                <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50 rounded-lg p-3 mb-4 flex items-center gap-3">
                  <Loader2 className="w-5 h-5 text-blue-600 animate-spin" />
                  <span className="text-sm text-blue-800 dark:text-blue-300 font-medium">
...
content = content.replace(old_extra_scan_logic, "")

# Find where the mapping loop ends and close the newly opened elements
content = content.replace("          )}", new_empty_logic)

# Replace the "124 ket qua" label logic
old_total_label = """{loading ? 'Đang tải...' : totalMentions >= 0 ? `${totalMentions.toLocaleString()} kết quả ${searchTerm ? `cho '${searchTerm}'` : ''}` : 'Đang tải...'}"""
new_total_label = """{loading && !mentionsList.length ? 'Đang tải kết quả đã lưu...' : totalMentions >= 0 ? `Tìm thấy ${totalMentions.toLocaleString()} kết quả ${searchTerm ? `cho '${searchTerm}'` : ''}` : 'Đang tải...'}"""
