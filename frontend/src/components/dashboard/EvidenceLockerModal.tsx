...
export default function EvidenceLockerModal({ isOpen, onClose, incident }: EvidenceLockerModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-2xl bg-slate-50 dark:bg-[#0B1220] rounded-2xl shadow-2xl border border-slate-200 dark:border-gray-800 flex flex-col max-h-[90vh]">
        
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-gray-800 bg-white dark:bg-[#111827] rounded-t-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="bg-white dark:bg-[#111827] border border-slate-300 dark:border-gray-700 p-5 rounded-xl space-y-4 shadow-sm relative overflow-hidden">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-[#0B1220] border border-slate-300 dark:border-gray-700 text-slate-900 dark:text-white text-sm rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all placeholder:text-gray-600"
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {items.map((item) => (
                <div key={item.id} className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-gray-800 p-4 rounded-xl flex items-start justify-between group hover:border-slate-300 dark:border-gray-700 transition-colors">
                  <div className="flex items-start space-x-3">
                    <div className="p-2 bg-slate-50 dark:bg-[#0B1220] rounded-lg border border-slate-200 dark:border-gray-800 mt-0.5">
                      <Link2 className="w-4 h-4 text-slate-500 dark:text-gray-400" />
                    </div>
                    <div>
                      <div className="flex items-center space-x-2">
                        <span className="text-xs font-bold text-slate-700 dark:text-gray-300 uppercase tracking-wide">{item.file_name}</span>
                      ...
                      {item.file_path && item.file_path !== 'Không có ghi chú' && (
                        <p className="text-xs text-slate-500 dark:text-gray-400 mt-2 bg-slate-50 dark:bg-[#0B1220] p-2 rounded border border-slate-200 dark:border-gray-800">
                          {item.file_path}
                        </p>
                      )}
                    </div>
