...
export default function CrisisWarRoomModal({ isOpen, onClose, incident }: CrisisWarRoomModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-5xl bg-[#0B1220] rounded-2xl shadow-2xl border border-gray-800 flex flex-col h-[90vh]">
        
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-gray-800 bg-[#111827] rounded-t-2xl shrink-0">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <div className="flex items-center space-x-3">
                <h2 className="text-xl font-bold text-white tracking-wide">Crisis War Room</h2>
                <span className="px-2 py-0.5 text-xs font-bold uppercase tracking-wider rounded bg-gray-800 text-gray-400 border border-gray-700">
                  INCIDENT #{incident.id}
                </span>
              </div>
              <p className="text-sm text-gray-400 mt-1 font-medium">{incident.title}</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            <div className="flex items-center justify-center h-64">
              <span className="text-gray-400 font-medium">Đang thiết lập phòng chỉ huy...</span>
            </div>
          ) : (
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-[#111827] border border-gray-800 rounded-xl p-5">
                    ...
                    <span className="text-lg font-bold text-white uppercase">{incident.status.replace('_', ' ')}</span>
                  </div>
                  <div className="bg-[#111827] border border-gray-800 rounded-xl p-5">
                ...
                {incident.description && (
                  <div className="bg-[#111827] border border-gray-800 rounded-xl p-5">
                    ...
                    <p className="text-sm text-gray-300 leading-relaxed bg-[#0B1220] p-4 rounded-lg border border-gray-800/50">
                      {incident.description}
                    </p>
                  </div>
                )}
                ...
                <div className="bg-[#111827] border border-gray-800 rounded-xl p-5">
                  ...
                  <div className="space-y-4">
                    <div className="relative border-l-2 border-indigo-500/30 ml-3 pl-5 py-2">
                      ...
                      <p className="text-sm font-medium text-white mt-1">Phát hiện và ghi nhận sự cố</p>
                    </div>
                    {logs.map((log) => (
                      <div key={log.id} className="relative border-l-2 border-gray-700 ml-3 pl-5 py-2">
                        ...
                        <p className="text-sm font-medium text-white mt-1 uppercase">{log.action}</p>
                        {log.notes && <p className="text-xs text-gray-400 mt-1">{log.notes}</p>}
                        {(log.old_status || log.new_status) && (
                          <div className="text-[10px] font-medium text-gray-500 mt-2 bg-[#0B1220] inline-block px-2 py-1 rounded border border-gray-800">
              ...
              <div className="space-y-6">
                ...
                {mention && mention.ai_analysis ? (
                  <div className="bg-[#111827] border border-gray-800 rounded-xl p-5">
                    ...
                    <div className="flex flex-col items-center justify-center p-4 bg-[#0B1220] rounded-xl border border-gray-800 mb-4">
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-center pb-2 border-b border-gray-800/50">
                        <span className="text-xs text-gray-400 font-medium">Crisis Level</span>
                        <CrisisLevelBadge level={mention.ai_analysis.crisis_level} />
                      </div>
                      <div className="flex justify-between items-center pb-2 border-b border-gray-800/50">
                        <span className="text-xs text-gray-400 font-medium">Sentiment</span>
                        <SentimentBadge sentiment={mention.ai_analysis.sentiment} />
                      </div>
                      <div className="flex justify-between items-center pb-2 border-b border-gray-800/50">
                        <span className="text-xs text-gray-400 font-medium">Khẩn cấp</span>
