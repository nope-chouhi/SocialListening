  ...
  return (
    <div className="bg-white dark:bg-[#050A15] rounded-xl shadow-sm border border-gray-200 dark:border-white/10 p-4 mt-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-sm font-bold text-gray-800 dark:text-gray-100">{t('mentions.antiNoiseTitle')}</h3>
      </div>
      
      <div className="space-y-3">
        {/* Supported capabilities */}
        <div className="text-xs">
          <span className="font-bold text-slate-700 dark:text-slate-300">{t('mentions.activeFeatures')}</span>
          <ul className="list-disc pl-4 mt-1 text-slate-500 dark:text-slate-400 space-y-0.5">
            <li>{t('mentions.filterByFeatures')}</li>
            <li>{t('mentions.removeSpam')}</li>
            <li>{t('mentions.blockDomainAuthor')}</li>
          </ul>
        </div>
