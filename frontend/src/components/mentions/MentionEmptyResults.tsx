      ...
      {searchState === 'TYPING' || searchState === 'SEARCHING_DB' ? (
        <>
          ...
          <p className="text-gray-600 dark:text-slate-500 dark:text-gray-400">
            {searchState === 'TYPING' ? 'Đang nhập từ khóa...' : `Đang tìm kiếm '${searchTerm}'...`}
          </p>
        </>
      ) : (
        <>
          ...
          <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-2">
            {dateRange === '1d' 
              ? 'Không có kết quả trong hôm nay.' 
              : searchTerm 
          ...
          {dateRange === '1d' ? (
            ...
            <p className="text-gray-600 dark:text-slate-500 dark:text-gray-400 mb-6 max-w-sm">
              Có thể dữ liệu nằm ngoài phạm vi thời gian hoặc bộ lọc quá nghiêm ngặt. Vui lòng nới lỏng bộ lọc hoặc quét thêm nguồn.
            </p>
          )}
